...
# ── C# System.Random port — fullShuffle ───────────────────────────────────
_MBIG = 2147483647
_INV  = 1.0 / 2147483647

def full_shuffle(num_levels, seed):
    """Port of C# System.Random-based Fisher-Yates shuffle used by Neon White."""
    SA = [0] * 56
...
class NeonWhiteApp:
    ...
    def _build_rush_finder(self):
        ...
        # Rush selector + search depth row
        ...
        self.finder_rush_var = tk.StringVar(value="White / Mikey (96 levels)")
        self._rush_dropdown(lc, self.finder_rush_var,
            ["White / Mikey (96 levels)", "Violet (8 levels)", "Red (8 levels)", "Yellow (8 levels)"])
        ...
        self._rush_entry(rc, var=self.finder_depth_var, width=10)
        ...
        tk.Label(body, text="Comma-separated level names. Case insensitive.",
                 font=("Helvetica", 9), bg=t["bg"], fg=t["fg2"]).pack(anchor="w", pady=(0, 8))
        ...
        tk.Label(body, textvariable=self.finder_status_var, font=("Helvetica", 9),
                 bg=t["bg"], fg=t["fg2"]).pack(anchor="w")

        # Results
        self._rush_field_label(body, "Results")
        result_frame = tk.Frame(body, bg=t["bg"])
        result_frame.pack(fill=tk.BOTH, expand=True)
        self.finder_result = self._rush_result_box(result_frame, height=8)

    def _rush_key_to_num(self, rush_var_str):
        mapping = {
            "White / Mikey (96 levels)": 96,
            "Violet (8 levels)": 8,
            "Red (8 levels)": 8,
            "Yellow (8 levels)": 8,
        }
    ...
    def _rush_key_from_display(self, display):
        mapping = {
            "White / Mikey (96 levels)": "96",
            "Violet (8 levels)": "violet",
            "Red (8 levels)": "red",
            "Yellow (8 levels)": "yellow",
        }
    ...
    def _run_finder(self):
        ...
        self._rush_show(self.finder_result, "")

        def manager_thread():
            result_queue = multiprocessing.Queue()
            workers      = []

            # Spawn one worker per core, each covering a chunk of the seed range
            ...
            while done_workers < num_cores:
                try:
                    ...
                    # Timeout — update progress and check stop flag
                    if self._finder_stop_event.is_set():
                        break
                    self.root.after(0, lambda f=len(found):
                        self.finder_status_var.set(
                            f"Searching {num_cores} core(s)... {f} found so far"
                        ))
                    continue

                if item is None:
                    # Sentinel from a finished worker
                    done_workers += 1
                    continue

                # Found a matching seed
                seed = item
                found.append(seed)
                order     = full_shuffle(num_levels, seed)
                positions = {ti: order.index(ti) + 1 for ti in target_indices if ti in order}
                match_str = ", ".join(
                    f"{names[ti]} @{positions[ti]}" for ti in target_indices
                )
                summary = "\n".join(
                    f"Seed {s}: " + ", ".join(
                        f"{names[ti]} @{full_shuffle(num_levels, s).index(ti)+1}"
                        for ti in target_indices
                    )
                    for s in found
                )
                self.root.after(0, lambda txt=summary: self._rush_show(self.finder_result, txt))
    ...
    def _build_rush_splits(self):
        ...
        self._rush_field_label(body, "Rush")
        self.splits_rush_var = tk.StringVar(value="White / Mikey (96 levels)")
        self._rush_dropdown(body, self.splits_rush_var,
            ["White / Mikey (96 levels)", "Violet (8 levels)", "Red (8 levels)", "Yellow (8 levels)"])

        self._rush_field_label(body, "Seed Number")
        self.splits_seed_var = tk.StringVar()
        self._rush_entry(body, var=self.splits_seed_var, width=20)
    ...
    def _build_rush_std(self):
        ...
        self._rush_field_label(body, "Rush")
        self.std_rush_var = tk.StringVar(value="White / Mikey (96 levels)")
        self._rush_dropdown(body, self.std_rush_var,
            ["White / Mikey (96 levels)", "Violet (8 levels)", "Red (8 levels)", "Yellow (8 levels)"])

        self._rush_field_label(body, "Seed Number")
        self.std_seed_var = tk.StringVar()
        self._rush_entry(body, var=self.std_seed_var, width=20)
    ...
    def _build_rush_timer(self):
        ...
        lc.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 12))
        self._rush_field_label(lc, "Rush")
        self.timer_rush_var = tk.StringVar(value="White / Mikey (96 levels)")
    ...
    def _apply_widget_defaults(self):
        ...
        self.root.option_add("*Button.Foreground", "#0a0a0f" if self.cfg["theme"] == "dark" else "#ffffff")
