...
def full_shuffle(num_levels, seed):
    ...
    for i in range(num_levels):
        ...
        if r < 0:      r += _MBIG
        SA[ie] = r
        # Integer-only index calculation — avoids float rounding that causes j >= num_levels
        j = (r % num_levels)
...
class NeonWhiteApp:
            ...
            while done_workers < num_cores:
                try:
                    ...
                    if self._finder_stop_event.is_set():
                        break
                    # Estimate progress from how many workers have finished
                    # plus a rough fraction based on time (can't know exact seed)
                    pct = min(99, int((done_workers / num_cores) * 100))
                ...
                if item is None:
                    done_workers += 1
                    pct = int((done_workers / num_cores) * 100)
                    self.root.after(0, lambda p=pct: self.finder_progress.configure(value=p))
                    continue
    ...
    def _apply_widget_defaults(self):
        ...
        style.configure("TCombobox",
                        fieldbackground=t["input_bg"], background=t["bg2"],
                        foreground=t["input_fg"], font=fnt,
