...
class NeonWhiteApp:
    ...
    def _add_row(self, key, rank, name, time_str):
        tree = getattr(self, f"{key}_tree")
        tree.insert("", tk.END, values=(f"#{rank}", name, f"{time_str}s"))
        ...
        for idx, (display, internal) in enumerate(LEVELS, 1):
            ...
            for r in level_rows:
                if out in ("display", "both"):
                    self._add_row("global", r["rank"], r["name"], r["time"])
                if writer:
    ...
    def _level_worker(self, display_name, internal_name, count, out):
        ...
        for r in all_rows:
            if out in ("display", "both"):
                self._add_row("level", r["rank"], r["name"], r["time"])
    ...
    def _player_worker(self, steam_id, levels_to_search, context, out):
        self._log("player", f"Looking up player across {len(levels_to_search)} levels...")
        rows = []
        ...
        for display_name, internal_name in levels_to_search:
            ...
            if entry:
                ...
                if out in ("display", "both"):
                    self._add_row("player", entry.global_rank, display_name, time_str)
            else:
