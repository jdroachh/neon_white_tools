...
class NeonWhiteApp:
    ...
    def _build_results_area(self, parent, key):
        ...
        cols = ("rank", "name", "time", "steam_id")
        tree = ttk.Treeview(table_frame, columns=cols, show="headings", height=10)
        for col, width, label in [
            ("rank", 70, "Rank"), ("name", 220, "Player"),
            ("time", 100, "Time"), ("steam_id", 180, "Steam ID")
        ]:
    ...
    def _add_row(self, key, rank, name, time_str, steam_id):
        tree = getattr(self, f"{key}_tree")
        tree.insert("", tk.END, values=(f"#{rank}", name, f"{time_str}s", steam_id))
    ...
    def _global_worker(self, count, folder, out):
        ...
        if out in ("csv", "both"):
            ...
            writer = csv.DictWriter(
                csv_file, fieldnames=["level","rank","steam_id","name","score_ms","time"]
            )
            writer.writeheader()
        ...
        for idx, (display, internal) in enumerate(LEVELS, 1):
            ...
            for r in level_rows:
                if out in ("display", "both"):
                    self._add_row("global", r["rank"], r["name"], r["time"], r["steam_id"])
                if writer:
                    writer.writerow(r)
            if csv_file:
                csv_file.flush()
    ...
    def _level_worker(self, display_name, internal_name, count, out):
        ...
        for r in all_rows:
            if out in ("display", "both"):
                self._add_row("level", r["rank"], r["name"], r["time"], r["steam_id"])
    ...
    def _player_worker(self, steam_id, levels_to_search, context, out):
        self._log("player", f"Looking up player {steam_id} across {len(levels_to_search)} levels...")
        rows = []
        ...
        for display_name, internal_name in levels_to_search:
            ...
            if entry:
                ...
                rows.append({
                    "level": display_name, "rank": entry.global_rank,
                    "time": time_str, "score_ms": entry.score, "total": total,
                    "steam_id": steam_id,
                })
                if out in ("display", "both"):
                    self._add_row("player", entry.global_rank, display_name,
                                  time_str, steam_id)
            else:
        ...
        if out in ("csv", "both") and rows:
            ...
            path = os.path.join(self.cfg["output_folder"], f"{steam_id}_{safe_ctx}.csv")
            with open(path, "w", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(
                    f, fieldnames=["level","rank","time","score_ms","total","steam_id"]
                )
                writer.writeheader()
