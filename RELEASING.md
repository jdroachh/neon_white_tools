...
Bump the version in **both** places — they drift apart if you only touch one
(the EXE's Properties → Details version silently lagged from 1.4.1 through 1.5.0
because this step used to list only `bridge.py`):

1. `SteamScraper/webview_app/bridge.py` — `APP_VERSION`:

   ```python
   APP_VERSION = "1.x.x"
   ```

2. `SteamScraper/version_info.txt` — the Win32 EXE metadata. Update **all four**:
   the `filevers` and `prodvers` quads (e.g. `(1, 5, 1, 0)`) and the
   `FileVersion` / `ProductVersion` StringStruct strings (e.g. `1.5.1.0`).

> **Sanity check after the build (step 3):** the built exe's
> `(Get-Item ...\NeonWhiteLeaderboardTool.exe).VersionInfo.FileVersion` should
> match the new version. If it still shows the old one, `version_info.txt`
> wasn't bumped.
...
- [ ] No tracebacks in the log. **As of 1.6.0 logs live in `%APPDATA%\NeonWhiteLeaderboardTool\logs\app.log`**, not the dist folder (`%APPDATA%` typically resolves to `C:\Users\<you>\AppData\Roaming`).

### 5. Clean runtime artifacts from the build

Smoke testing can drop user-specific files inside the dist folder. Remove them
before zipping so they don't ship to other users (run from the repo root):

```powershell
Remove-Item "SteamScraper\dist\NeonWhiteLeaderboardTool\steam_appid.txt" -ErrorAction SilentlyContinue
# Defensive only — pre-1.6.0 builds also wrote these into dist; harmless no-ops on 1.6.0+:
Remove-Item "SteamScraper\dist\NeonWhiteLeaderboardTool\neonwhite_config.json" -ErrorAction SilentlyContinue
Remove-Item "SteamScraper\dist\NeonWhiteLeaderboardTool\logs" -Recurse -Force -ErrorAction SilentlyContinue
```

> **Changed in 1.6.0:** `neonwhite_config.json` and `logs/` now live in
> `%APPDATA%\NeonWhiteLeaderboardTool\`, **not** the dist folder — so the only
> artifact smoke testing leaves beside the exe is `steam_appid.txt`. Do **not**
> delete the `%APPDATA%` config as part of cleanup; that's your own real saved
> data, and it's outside the build anyway. The two defensive removals above just
> cover stale pre-1.6.0 dist folders.

All are regenerated by the app on first launch (config + logs in `%APPDATA%`).

### 6. Zip the build

Make sure the app is fully closed first (it keeps `logs/app.log` open). Run from
the repo root:

```powershell
Compress-Archive `
  -Path "SteamScraper\dist\NeonWhiteLeaderboardTool" `
  -DestinationPath "NeonWhiteLeaderboardTool-1.x.x.zip" `
  -Force
```
