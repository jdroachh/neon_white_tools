...
| [`sheets.py`](../SteamScraper/sheets.py)                         | Google Sheets push for Player Lookup data. Owns: `get_sheets_service`, `push_to_sheet`, `parse_cell`, `col_letter_to_index` + `SHEETS_SCOPE`, `CREDENTIALS_FILE`, `TOKEN_FILE`. **Access pattern: lazy import** — callers in `neonwhite_app.py` write `import sheets` *inside* the function that needs it (`auth_worker`, `push_worker`), NOT at module top. Module load is cheap; the slow Google client libraries (~217ms cold) only load when the user actually clicks Sign In or Push. `neonwhite_app.py` keeps its own `TOKEN_FILE = "token.json"` constant for UI status checks to avoid triggering the lazy import at launch. |
...
| [`neonwhite_config.json`](../neonwhite_config.json) | User config — lives at **repo root** (not `SteamScraper/`). Keys: `dll_path`, `output_folder`, `entry_count`, `accent_color`, `saved_profiles`, `guide_watchlist`, `guide_watched`, `guide_hide_watched`, `guide_watchlist_only`. All reads/writes serialised by `_CONFIG_LOCK` in `bridge.py`. |
